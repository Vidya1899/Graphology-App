...
def main(src_img):
	...
	# src_img = skewcorrection(src_img)
	best_angle, src_img = correct_skew(src_img)
	...
	print('ul,ll:',upperlines, lowerlines)
	if len(upperlines)==len(lowerlines):
		# if lower_lines > final_thr.shape[0]:

		lines = []
		for y in upperlines:
			print(y)
			print(final_thr.shape[0],final_thr.shape[1])
			# print(final_thr[y][:])
			final_thr[y][:] = 255
		for y in lowerlines:
			if y > final_thr.shape[0]:
				y = final_thr.shape[0]-1
			print('now y :',y)
			y -= 1
			final_thr[y][:] = 255
